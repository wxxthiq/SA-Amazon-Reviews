...
def main():
    ...
    if 'explorer_date_filter' not in st.session_state: st.session_state.explorer_date_filter = default_date_range
    if 'explorer_rating_filter' not in st.session_state: st.session_state.explorer_rating_filter = default_ratings
    if 'explorer_sentiment_filter' not in st.session_state: st.session_state.explorer_sentiment_filter = default_sentiments
    if 'explorer_verified_filter' not in st.session_state: st.session_state.explorer_verified_filter = default_verified
    ...
    st.sidebar.multiselect("Filter by Star Rating", options=default_ratings, key='explorer_rating_filter', on_change=reset_page_number)
    st.sidebar.multiselect("Filter by Sentiment", options=default_sentiments, key='explorer_sentiment_filter', on_change=reset_page_number)
    ...
    # 1. Fetch the small, paginated data for display
    paginated_reviews_df, total_reviews = get_paginated_reviews(
        ...
        sort_by=sort_by, limit=REVIEWS_PER_PAGE, offset=st.session_state.review_page * REVIEWS_PER_PAGE
    )
    
    # 2. Fetch the full filtered data ONCE for both export and plotting.
    #    The @st.cache_data decorator on the function is crucial here.
    all_filtered_df = get_all_filtered_reviews(
        _conn=conn, asin=selected_asin, date_range=st.session_state.explorer_date_filter,
        rating_filter=tuple(st.session_state.explorer_rating_filter), sentiment_filter=tuple(st.session_state.explorer_sentiment_filter),
        verified_filter=st.session_state.explorer_verified_filter, search_term=st.session_state.explorer_search_term
    )
    ...
    if paginated_reviews_df.empty:
        ...
        with export_c2:
            st.download_button(
               label="📥 Export to CSV",
               data=convert_df_to_csv(all_filtered_df), # Use the pre-fetched data
    ...
    # --- OPTIMIZED ADVANCED ANALYSIS PLOT ---
    st.markdown("---")
    with st.expander("🔬 Advanced Analysis: Find Mismatched Reviews"):
        st.markdown("##### What is this?")
        st.markdown("This scatter plot helps you find reviews where the **star rating might not match the sentiment of the written text**.")
        st.markdown("##### How do I use it?")
        st.markdown("Each dot is a review. **Click on any dot** to see the full review text on the right. The color indicates the 'Discrepancy Score'—brighter dots have a bigger mismatch.")
        
        # Use the pre-fetched `all_filtered_df` DataFrame
        if all_filtered_df.empty:
            st.warning("No review data available for the selected filters to generate this plot.")
        else:
            # This processing now only happens when the expander is OPEN
            with st.spinner("Generating plot..."):
                all_filtered_df_for_plot = all_filtered_df.copy() # Create a copy to avoid modifying the original
                ...
                with plot_col:
                    ...
                    if selected_points and 'pointIndex' in selected_points[0]:
                        ...
                        if point_index < len(all_filtered_df_for_plot):
                            ...
                            if st.session_state.get('selected_review_id') != clicked_id:
                                st.session_state.selected_review_id = clicked_id
                                st.rerun()
                with review_col:
                    if 'selected_review_id' in st.session_state and st.session_state.selected_review_id:
