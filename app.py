...
# --- New Imports for Kaggle Downloader ---
import os
...
# --- Configuration ---
# This is now just the file name, as it will be downloaded to the current directory.
DATABASE_PATH = "amazon_fashion.db" 
PRODUCTS_PER_PAGE = 16

# --- KAGGLE DATASET DOWNLOADER ---
def download_from_kaggle(dataset_slug, file_name):
    """
    Checks for a file and downloads it from a private Kaggle dataset if not present.
    Uses st.secrets for authentication.
    """
    ...
    st.info(f"Database '{file_name}' not found. Initializing download from Kaggle...")
    ...
    with open(kaggle_json_path, "w") as f:
        f.write(f'{{"username":"{st.secrets["kaggle"]["username"]}", "key":"{st.secrets["kaggle"]["key"]}"}}')
    
    # Set permissions for the file to be readable only by the owner
    os.chmod(kaggle_json_path, 0o600)

    # --- Initialize and use the Kaggle API ---
    with st.spinner("Connecting to Kaggle and downloading dataset... Please wait."):
        try:
            api = KaggleApi()
            api.authenticate()

            # Download the dataset files
            # NOTE: Assumes the .db file is inside a .zip archive, which is common.
            ...
            if os.path.exists(file_name):
                ...
                st.error(f"Download complete, but the expected file '{file_name}' was not found. Please check the contents of your Kaggle dataset.")

        except Exception as e:
            st.error(f"An error occurred while downloading from Kaggle: {e}")
            st.stop()


# --- Main App ---
st.set_page_config(layout="wide", page_title="Amazon Review Explorer")
st.title("⚡ Advanced Sentiment Dashboard")

# --- KAGGLE DATASET INFO ---
# IMPORTANT: Replace these with your Kaggle username and dataset slug
KAGGLE_USERNAME = "your-kaggle-username"
KAGGLE_DATASET_SLUG = "your-private-dataset-slug"
KAGGLE_DATASET = f"{KAGGLE_USERNAME}/{KAGGLE_DATASET_SLUG}"

# --- Download data at the start of the app run ---
download_from_kaggle(KAGGLE_DATASET, DATABASE_PATH)


# --- Your existing code continues from here ---
# (The rest of your app.py file remains the same, but I'm including it
#  all below to ensure you have a complete, working script)
...
if st.session_state.selected_product:
    # --- DETAILED PRODUCT VIEW CODE ---
    # (This entire block remains the same as the last version you had)
    ...
    if not product_reviews.empty and nlp is not None:
        ...
        with keyword_tab:
            ...
            if len(selected_date_range) == 2:
                dashboard_filtered_reviews = dashboard_filtered_reviews[(dashboard_filtered_reviews['timestamp'].dt.date >= selected_date_range[0]) & (dashboard_filtered_reviews['timestamp'].dt.date <= selected_date_range[1])]
            with wordcloud_col:
