...
# --- New Imports for simplified downloader ---
import os
import requests
import zipfile
...
# --- Configuration ---
# IMPORTANT: Make sure these match your Kaggle username and public dataset slug
KAGGLE_USERNAME = "wathiqsoualhi"
KAGGLE_DATASET_SLUG = "db-fashion-reviews"


# IMPORTANT: Make sure this is the exact name of the .db file inside the zip archive
DATABASE_PATH = "amazon_reviews.db" 
PRODUCTS_PER_PAGE = 16

# --- Simplified Public Data Downloader ---
def download_and_unzip_from_kaggle(dataset_slug, file_name_to_check):
    """
    Downloads and unzips a public Kaggle dataset if the target file isn't present.
    """
    if os.path.exists(file_name_to_check):
        return

    st.info(f"Database '{file_name_to_check}' not found. Downloading from public Kaggle dataset...")

    # Construct the public download URL
    url = f"https://www.kaggle.com/api/v1/datasets/download/{dataset_slug}"
    zip_file_path = "archive.zip"

    with st.spinner(f"Downloading data from {url}..."):
        try:
            # Download the file
            ...
            if os.path.exists(file_name_to_check):
                ...
                st.error(f"Download complete, but the expected file '{file_name_to_check}' was not found after unzipping. Please check the dataset contents.")
                st.stop()

        except Exception as e:
            st.error(f"An error occurred during download and extraction: {e}")
            st.stop()
...
# --- Construct dataset slug and run downloader ---
KAGGLE_DATASET = f"{KAGGLE_USERNAME}/{KAGGLE_DATASET_SLUG}"
download_and_unzip_from_kaggle(KAGGLE_DATASET, DATABASE_PATH)
...
if st.session_state.selected_product:
    ...
    # (This entire block remains the same as the last working version)
    ...
    # (The rest of your code for the search page remains unchanged)
    # ...
